...
- Third-party connectors with vault-sourced tokens, tier-mapped per the PRD connector table: GitHub, Slack, Notion, Todoist, Linear (raw-key/GraphQL), Google (Calendar + Drive via OAuth2 refresh-token), and Microsoft Graph (Outlook mail/calendar, OneDrive, send-mail). All credential-free testable; tokens stored in the Vault under fixed secret names
...
- Raw screenshot/OCR artifact auto-expiry (PRD §9.6) and agent-supervisor root tool-call budget propagation across a workflow (PRD §7)
- 2026-06 security hardening: owner-auth session reuse/GC + optional PBKDF2 passphrase, Zone-3 trust-envelope provenance, Tier-3 auto-approval lockout, secrets-vault scheme tagging (see `docs/AUDIT_RESPONSE_2026-06-21.md`); per-track log in `docs/PRD_PROGRESS_2026-06-27.md`
...
- The one-time OAuth consent UX for Google/Microsoft (the connectors are built; the owner stores a refresh token in the Vault out-of-band to activate them)
- AES-256 full-database encryption at rest (PRD §9.6) — needs the SQLCipher dependency; today only the secrets vault is DPAPI-encrypted
